...
static  NSString * const operationQueueName = @"CMSS.SYUnitTestsDemoNetworking.queue";
...
+ (instancetype)shareInstance {
    static SYNetworkManager *shareInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareInstance = [[SYNetworkManager alloc] init];
    });
...
- (NSDictionary *)buildFakeResponseWithData:(id)data{
    NSString *code = @"200";
    NSMutableDictionary *res = [[NSMutableDictionary alloc] initWithDictionary:@{@"code":code,@"data":data?:@"fake data"}];
    return res;
}

- (NSDictionary *)buildFailureResponseWithMsg:(NSString *)msg{
    NSString *code = @"300";
    NSMutableDictionary *res = [[NSMutableDictionary alloc] initWithDictionary:@{@"code":code,@"msg":msg?:@"request failure"}];
    return res;
